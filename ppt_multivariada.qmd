...
packages <- c("GGally", "tidyverse", "corrplot", "reshape2", "viridisLite",
              "viridis", "RColorBrewer", "igraph", "ggraph", "plotly", "ggExtra", "reticulate", "hrbrthemes", "ggiraph", "ellipse", "fmsb", "scales", "plotly", "MASS", "mvtnorm")
...
densidade <- MASS::kde2d(ataque, defesa, n = 40)

densidade_total <- sum(densidade$z)

densidade_normalizada <- densidade$z / densidade_total

plot_ly(x = densidade$x, y = densidade$y, z = densidade_normalizada) %>%
  add_surface(colorscale = "Cividis") %>%
  layout(
    scene = list(
      xaxis = list(title = "Ataque"),
      yaxis = list(title = "Defesa"),
